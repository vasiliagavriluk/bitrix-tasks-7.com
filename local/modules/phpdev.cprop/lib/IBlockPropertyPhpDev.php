...
class IBlockPropertyPhpDev
{
    ...
    public static function GetUserTypeDescription()
    {
        ...
        return array(
            ...
            'CLASS_NAME' => "IBlockPropertyPhpDev",
            // базовый тип на котором будут основаны операции фильтра
            'BASE_TYPE' => 'int',
            // название для показа в списке типов пользовательских свойств
            'DESCRIPTION' => Loc::getMessage('IBLOCK_CPROP_DESCRIPTION'), //НАЗВАНИЕ В ИНФОБЛОК->СВОЙСТВА->ТИП
    ...
    /**
     * HTML отображения элемента управления для редактирования значений свойства
     * Вывод всех свойств в элементе инфоблока
     * @param $arProperty
     * @param $value
    ...
    {
        $hideText = Loc::getMessage('HIDE_TEXT');
        $clearText = Loc::getMessage('CLEAR_TEXT');
        ...
        if(!empty($arProperty['USER_TYPE_SETTINGS'])){
            ...
            return '<span>'.Loc::getMessage('ERROR_INCORRECT_SETTINGS').'</span>';
        }
        ...
        foreach ($arFields as $code => $arItem){
            ...
            else if($arItem['TYPE'] === 'html_edit'){

                $result .= self::showHTMLEdit($code, $arItem['TITLE'], $value, $strHTMLControlName);
            }
        }
    ...
    {
        $btnAdd = Loc::getMessage('SETTING_BTN_ADD');
        $settingsTitle =  Loc::getMessage('SETTINGS_TITLE');

        $arPropertyFields = array(
            'USER_TYPE_SETTINGS_TITLE' => $settingsTitle,
            'HIDE' => array('ROW_COUNT', 'COL_COUNT', 'DEFAULT_VALUE', 'SEARCHABLE', 'SMART_FILTER',
                'WITH_DESCRIPTION', 'FILTRABLE', 'MULTIPLE_CNT', 'IS_REQUIRED'),
            'SET' => array(
                'MULTIPLE_CNT' => 1,
                'SMART_FILTER' => 'N',
                'FILTRABLE' => 'N',
        ...
        $result = '<tr><td colspan="2" align="center">
            <table id="many-fields-table" class="many-fields-table internal">        
                <tr valign="top" class="heading mf-setting-title">
                   <td>XML_ID</td>
                   <td>'.Loc::getMessage('SETTING_FIELD_TITLE').'</td>
                   <td>'.Loc::getMessage('SETTING_FIELD_SORT').'</td>
                   <td>'.Loc::getMessage('SETTING_FIELD_TYPE').'</td>
                </tr>';
    ...
    /**
     * Метод возвращает безопасный HTML отображения значения свойства в публичной части сайта.
     * @param $arProperty
     * @param $value
     * @param $strHTMLControlName
     * @return mixed
    ...
    /**
     * Метод преобразовывает значение свойства в формат для сохранения в базе данных
    ...
    /**
     * Метод возвращает файл для базы
     * @param $arValue
    ...
    /**
     * Метод возвращает массив с пользовательских свойствами
     * @param $arProperty
     * @return array
     */
    ...
    /**
     * Метод вернет фактическую длину значения свойства.
     * @param $arProperty
     * @param $arValue
     * @return bool
     */
    ...
    private static function getOptionList($selected = 'string')
    {
        $result = '';
        $arOption = [
            'string' => Loc::getMessage('FIELD_TYPE_STRING'),
            'file' => Loc::getMessage('FIELD_TYPE_FILE'),
            'text' => Loc::getMessage('FIELD_TYPE_TEXT'),
            'date' => Loc::getMessage('FIELD_TYPE_DATE'),
            'element' => Loc::getMessage('FIELD_TYPE_ELEMENT'),
            'html_edit' => Loc::getMessage('FIELD_TYPE_HTML_EDIT')
        ];
    ...
    //Отрисовка (что видит пользователь в админке\добавление элемента)
                                ...
                                <tr>
                                    <td style="padding: 0;">
                                        <div class="adm-input-wrap adm-input-wrap-calendar">
                                            ...
                                            name="'.$strHTMLControlName['VALUE'].'['.$code.']" size="23" value="'.$v.'">
                                            <span class="adm-calendar-icon"
                                                  onclick="BX.calendar({node: this, 
                                                  field:\''.$strHTMLControlName['VALUE'].'['.$code.']\', form: \'\', 
                                                  bTime: true, bHideTime: false});"></span>
                                        </div>
                                    </td>
                                </tr>
                            </table>
    ...
    public static function showHTMLEdit($code, $title, $arValue, $strHTMLControlName)
    {
        ob_start();
        echo '<div class="html_realweb">';
        echo '<tr><td align="right" valign="top">'.$title.': </td><td>';
        echo '<textarea class="typearea" rows="8" 
                style="display:none;"
                id="bxed_'.$code.'" wrap="virtual"
                name="'.$strHTMLControlName['VALUE'].'['.$code.']">'. $arValue["VALUE"][$code].'</textarea>';
